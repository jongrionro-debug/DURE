...
3. 기관 생성 온보딩, 기본 운영 설정, create-first 세션 생성, 운영자 세션 관리, 강사 기록 제출까지 로컬에서 검증 가능할 것
...
문서 전반에서 사용자에게 보이는 한국어 명칭은 `사업`을 기본으로 사용한다. 다만 현재 구현과 스키마에서는 범용 식별자 호환성을 위해 내부 필드명 `programs`, `programId`를 그대로 유지할 수 있다.
...
| 기관 운영자 | 자기 기관의 운영 시스템을 생성하고 운영 기록을 관리한다 | 기관 생성 온보딩, 제출 현황 확인, 사용자/권한 관리, 운영 기본정보 설정, 세션 생성/세션 관리, 기록 검색/필터링, 상세 조회 |
...
- 자기 기관의 세션별 강사 할당, 강사 할당 해제, 참여자 추가를 수행할 수 있다.
...
4. 기관 운영자 경험의 중심은 `생성 완료 -> 운영 기본정보 설정 -> 대시보드 -> 세션 관리 -> 기록 관리 -> 사용자/권한`이다.
...
| OA-05 | 세션 관리 | 기관 운영자 | Must |
| OA-06 | 통계/성과 | 기관 운영자 | Should |
...
2. 참여자 snapshot이 1명도 없는 세션은 제출할 수 없고, 운영자에게 세션 관리에서 참여자를 먼저 추가하라고 안내해야 한다.
3. 첨부 문서 업로드 실패는 명확히 안내해야 한다.
4. 첨부 문서가 없어도 제출은 가능해야 한다.
5. 입력 중 이탈 시 손실 가능성을 사용자에게 안내할지 여부는 구현 시 UX 세부사항으로 정한다.
...
5. 강사 미할당 세션도 운영자의 후속 조치를 위해 대시보드 목록에서 계속 보여야 한다.
6. 최근 제출/미제출 목록에서 운영자는 세션 관리 화면으로 바로 이동할 수 있어야 한다.
7. 대시보드는 기록 관리 상세 화면으로 들어가기 전 요약 화면 역할에 집중해야 한다.
8. 기관 운영자는 자기 기관 기준 데이터만 보아야 한다.
...
4. 로컬 MVP에서는 실제 메일 발송 대신 공유 가능한 전체 초대 토큰을 생성하고 복사할 수 있어야 한다.
5. 권한 변경은 즉시 반영되어야 한다.
...
5. 기록 상세 화면에서는 같은 세션의 세션 관리 화면으로 바로 이동할 수 있어야 한다.
...
7. 세션 생성 시 강사와 참여자 명단이 비어 있어도 생성 자체는 허용되어야 한다.
8. 세션 생성 시 해당 세션의 출석 대상 명단은 복사되어 고정되어야 한다.
9. 설정 데이터는 자기 기관 범위 안에서만 유효해야 한다.
10. 사업이 하나도 없는 경우 `첫 사업 만들기`가 우선 액션으로 노출되어야 한다.

### OA-05 세션 관리

**목적**

기관 운영자가 create-first로 생성한 세션의 부족한 정보를 보완하고, 강사 제출 전후 상태를 세션 단위로 관리할 수 있어야 한다.
...
1. 운영자는 강사 미할당 세션에도 접근할 수 있어야 한다.
2. 운영자는 승인된 teacher만 세션 강사로 할당할 수 있어야 한다.
3. 운영자는 빈 값 저장으로 세션 강사 할당을 해제할 수 있어야 한다.
4. 운영자는 전체 참여자 명단의 기존 참여자를 세션 snapshot에 추가할 수 있어야 한다.
5. 운영자는 새 참여자를 생성하면서 전체 참여자 명단과 세션 snapshot에 동시에 반영할 수 있어야 한다.
6. 같은 참여자를 같은 세션에 중복 추가할 수 없어야 한다.
7. 제출이 끝난 세션에 참여자가 추가되면, 강사가 다시 수정 저장하기 전까지 해당 출석 상태는 `미입력`으로 보여야 한다.
8. 참여자가 0명인 세션은 생성할 수 있지만, 강사는 최종 제출할 수 없어야 한다.
9. 세션 관리 화면은 강사 제출 작업공간과 분리된 운영자 전용 화면이어야 한다.
